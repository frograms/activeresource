  ...
  module CustomMethods
    ...
    module ClassMethods
      def custom_method_collection_url(method_name, options = {})
        ...
        "#{prefix(prefix_options)}#{collection_name}#{method_name.present? ? "/#{method_name}" : nil}#{format_extension}#{query_string(query_options)}"
      end
    end
    ...
    private
      def custom_method_element_url(method_name, options = {})
        "#{self.class.prefix(prefix_options)}#{self.class.collection_name}/#{id}#{method_name.present? ? "/#{method_name}" : nil}#{self.class.format_extension}#{self.class.__send__(:query_string, options)}"
      end

      def custom_method_new_element_url(method_name, options = {})
        "#{self.class.prefix(prefix_options)}#{self.class.collection_name}/new#{method_name.present? ? "/#{method_name}" : nil}#{self.class.format_extension}#{self.class.__send__(:query_string, options)}"
      end
  end
end
